...
export default function Antennas() {
  ...
  let antennas: [AccessPoint] = reduceAPs();
  let reducedAntennas: ReducedPoints = {};

  for(let i = 0; i < antennas.length; i++) {
    let val: string = `${antennas[i].lat} ${antennas[i].lon}`
  ...
  return Object.entries(reducedAntennas).map((value: [string, ReducedContent]) => (
